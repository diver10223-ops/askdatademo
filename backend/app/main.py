...
class ProviderProfileIn(BaseModel):
 ...
 allowed_tables:list[str]=Field(min_length=1); allowed_columns:list[str]=Field(default_factory=lambda:sorted(DEFAULT_ANALYTIC_COLUMNS),min_length=1); datasource_read_only:bool=True; datasource_charset:str='utf8mb4'; datasource_pool_size:int=Field(default=5,ge=1,le=50); schema_sync_enabled:bool=True; max_rows:int=Field(default=1000,ge=1,le=10000); max_time_range_days:int=Field(default=366,ge=1,le=3660); max_estimated_scan_rows:int=Field(default=1000000,ge=1); timeout:float=Field(default=30,ge=1,le=120); retries:int=Field(default=2,ge=0,le=5); backoff:float=Field(default=.25,ge=0,le=10); max_concurrency:int=Field(default=4,ge=1,le=32)
...
class DatasourceConfigIn(BaseModel):
 name:str=Field(min_length=1,max_length=100); type:str=Field(pattern='^(CLICKHOUSE|MYSQL)$'); url:str|None=None; host:str|None=None; port:int=3306; tls:bool=True; username:str; password:str=Field(min_length=1); database:str; allowed_tables:list[str]=Field(min_length=1); allowed_columns:list[str]=Field(default_factory=lambda:sorted(DEFAULT_ANALYTIC_COLUMNS),min_length=1); read_only:bool=True; charset:str='utf8mb4'; pool_size:int=Field(default=5,ge=1,le=50); schema_sync:bool=True; max_rows:int=Field(default=1000,ge=1,le=10000); max_time_range_days:int=Field(default=366,ge=1,le=3660); max_estimated_scan_rows:int=Field(default=1000000,ge=1); timeout:float=Field(default=30,ge=1,le=120); retries:int=Field(default=2,ge=0,le=5); max_concurrency:int=Field(default=4,ge=1,le=32)
...
async def diagnose_datasource_config(rid:str):
 ...
 cfg=json.loads(row['public_config']); secret=decrypt_secret(row['encrypted_credentials']); retry=RetryPolicy(cfg['timeout'],cfg['retries'],.25,cfg['max_concurrency']); dialect='clickhouse' if row['type']=='CLICKHOUSE' else 'mysql'; policy=SQLPolicy(frozenset(cfg['allowed_tables']),cfg['max_rows'],cfg['max_time_range_days'],retry.timeout,dialect,frozenset(cfg.get('allowed_columns',DEFAULT_ANALYTIC_COLUMNS)),max_estimated_scan_rows=cfg.get('max_estimated_scan_rows',1000000),require_scope_permissions=True)
...
def compose_phase2_profile(body:ProviderCompositionIn):
 with connect(PLATFORM_DB) as db:
  ...
  public={'model_provider':model['provider'],'model_base_url':mc['base_url'],'model':mc['model'],'model_temperature':mc['temperature'],'model_top_p':mc['top_p'],'model_max_tokens':mc['max_tokens'],'model_context_window':mc['context_window'],'model_structured_output':mc['structured_output'],'model_system_prompt':mc['system_prompt'],'datasource_url':dc.get('url'),'datasource_host':dc.get('host'),'datasource_port':dc.get('port',3306),'datasource_tls':dc.get('tls',True),'datasource_username':dc['username'],'database':dc['database'],'allowed_tables':dc['allowed_tables'],'allowed_columns':dc.get('allowed_columns',sorted(DEFAULT_ANALYTIC_COLUMNS)),'datasource_read_only':dc['read_only'],'datasource_charset':dc['charset'],'datasource_pool_size':dc['pool_size'],'schema_sync_enabled':dc['schema_sync'],'max_rows':dc['max_rows'],'max_time_range_days':dc['max_time_range_days'],'max_estimated_scan_rows':dc.get('max_estimated_scan_rows',1000000),'timeout':max(mc['timeout'],dc['timeout']),'retries':max(mc['retries'],dc['retries']),'max_concurrency':min(mc['max_concurrency'],dc['max_concurrency']),'model_config_id':model['id'],'datasource_config_id':source['id']}
