...
CREATE TABLE IF NOT EXISTS internal_session_identity(session_id TEXT PRIMARY KEY REFERENCES sessions(id),subject_id TEXT NOT NULL,role_snapshot TEXT NOT NULL,permission_digest TEXT NOT NULL,config_version_id TEXT NOT NULL);
...
CREATE TABLE IF NOT EXISTS internal_execution_idempotency(idempotency_key TEXT PRIMARY KEY,request_id TEXT NOT NULL UNIQUE REFERENCES requests(id),command_digest TEXT NOT NULL,created_at TEXT NOT NULL);
