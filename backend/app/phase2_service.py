...
def public_profile(row) -> dict:
    result=dict(row); result["public_config"]=json.loads(result["public_config"]); result.pop("encrypted_credentials",None); return result
...
def build_registry(profile_id: str, require_enabled: bool = True) -> Phase2ProviderRegistry:
    ...
    model=OpenAICompatibleProvider(config["model_base_url"],secret["model_api_key"],config["model"],retry)
