...
class Engine:
 def __init__(self):
  self.registry=ProviderRegistry(); self.layers=[InteractionLayer(),UnderstandingLayer(),SemanticLayer(),AssetLayer(),QueryLayer(),ExecutionLayer(self.registry),InterpretationLayer(self.registry)]
 ...
 async def run(self,c):
  ...
  for layer in self.layers:
   with connect(PLATFORM_DB) as db:
    if db.execute('SELECT cancel_requested FROM requests WHERE id=?',(c.request_id,)).fetchone()[0]:
     c.status='CANCELLED'; c.termination_reason='CANCELLED'; self.event(c,'request.cancelled',{'layer':layer.layer_code}); break
   ...
   with connect(PLATFORM_DB) as db:
    db.execute('UPDATE layer_executions SET status=?,output_json=?,provider=?,elapsed_ms=?,error_code=? WHERE request_id=? AND layer_code=?',(result.status,json.dumps(result.output,ensure_ascii=False),'MOCK' if layer.layer_code in ('L2','L7') else ('SQLITE' if layer.layer_code=='L6' else 'DETERMINISTIC'),elapsed,result.error_code,c.request_id,layer.layer_code))
    if layer.layer_code=='L6':
     for i,p in enumerate(c.sql_plan,1): db.execute('INSERT INTO sql_executions(request_id,sequence,business_sql,actual_sql,parameters,source,status,row_count,elapsed_ms,fallback) VALUES(?,?,?,?,?,?,?,?,?,?)',(c.request_id,i,p['business_sql'],p['actual_sql'],json.dumps(p['parameters'],ensure_ascii=False),p['source'],'SUCCEEDED',len(c.results),elapsed,1 if p['source']=='MOCK_FIXTURE' else 0))
