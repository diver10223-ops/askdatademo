...
public class ConfigurationReleaseService {
    ...
    public ConfigurationReleaseService(JdbcTemplate jdbc, ApprovalService approvals, AuditLogService audit) {
        this.jdbc = jdbc;
    ...
    @Transactional
    public void markReviewing(long releaseId, long actorId) {
        ...
        audit.append(event(actorId, "SUBMIT_CONFIG_REVIEW", releaseId, "DRAFT", "REVIEWING"));
    }
    ...
    public PublishedRelease publish(long releaseId, long approvalId, long actorId, String environment) {
        ...
        audit.append(event(actorId, "PUBLISH_CONFIG", releaseId, "REVIEWING", "PUBLISHED"));
    ...
    public PublishedRelease rollback(long targetReleaseId, long actorId, String environment) {
        ...
        audit.append(event(actorId, "ROLLBACK_CONFIG", rollbackId, String.valueOf(targetReleaseId), "PUBLISHED"));
    ...
    private AuditLogService.AuditEvent event(long actorId, String action, long releaseId, String from, String to) {
        return new AuditLogService.AuditEvent("config-" + UUID.randomUUID(), actorId, "user-" + actorId, action,
                "CFG_RELEASE", String.valueOf(releaseId), "{\"status\":\"" + from + "\"}", "{\"status\":\"" + to + "\"}", null, null, "SUCCEEDED", null);
    }
