...
| 6 | M1—M7 产品基线 | IN PROGRESS | P372短时容量及真实30分钟峰值已通过，8小时长稳待执行 |
| 7 | M8—M9 客户交付 | BLOCKED | 等待具体客户环境和验收输入 |

P300—P303已完成，M0门禁通过。稳定演示工作树位于`/workspaces/askdatademo-v1-v2-demo`，V2.0开发工作树位于`/workspaces/askdatademo`；M1—M6及P370—P371已完成，P372短时容量与30分钟峰值通过，8小时长稳待执行。
...
| 日期与执行人 | 2026-08-12；Codex执行 |
| 验证命令 | `JAVA_HOME=/tmp/askdata-jdk21 PATH=/tmp/askdata-jdk21/bin:$PATH ./platform-service/mvnw -f platform-service/pom.xml -q test` |
| 证据 | Flyway V8新增指标映射、维度成员/映射、同义词、推荐和血缘；官方种子对账为1数据源、1表、8字段、3指标映射、2维度映射、2同义词、7推荐、3血缘边；`AssetManagementApiTests`经V2 API创建/查询规范化指标、写入审计并拒绝非法编码；Java总计15 tests、0 failures |
| 结论 | 数据源、表、字段、指标、维度、映射、优先级、推荐、同义词、血缘和敏感等级均有规范化事实表；V2管理API不修改官方JSON |
...
| 任务ID | P364 |
| 状态 | PASS |
| 分支与提交 | `release/product-v2-test`；恢复实现提交`7d6f9bbfa2480710fd91abb3bda17129a3b623d0`；记录提交以本记录所在提交为准 |
| 日期与执行人 | 2026-08-12；Codex执行 |
| 验证命令 | Java专项`./platform-service/mvnw -f platform-service/pom.xml -q -Dtest=DatabaseRecoveryToolTests test`；Java全量`./platform-service/mvnw -f platform-service/pom.xml -q test`；干净提交执行`JAVA_HOME=/tmp/askdata-jdk21 PATH=/tmp/askdata-jdk21/bin:$PATH bash scripts/v2-disaster-recovery-drill.sh` |
| 证据 | 恢复包强制包含数据库、数据库清单、匹配发布JAR、加密秘密恢复材料、Manifest和SHA256；数据库清单覆盖Flyway版本、配置/当前发布数量、配置快照摘要、秘密引用数量/摘要、请求和审计数量；恢复前校验全部组件、摘要、`RESTORE:<backupId>`人工确认、应用版本和空目标库，恢复后逐项重算清单；专项验证非空目标提前拒绝；从干净提交`7d6f9bb`构建发布JAR，在两个隔离文件库完成迁移15、非空配置快照/秘密引用备份与恢复，五项文件摘要、恢复清单、发布JAR和秘密材料逐字节一致；恢复包`v2-20260812T093943Z-0c8e9e41`演练PASS，且缺秘密材料、版本不匹配均被拒绝；Java全量32 tests通过 |
| 结论 | 产品H2基线可从不可拆分恢复包复现匹配的数据库、配置快照、秘密引用关系、迁移版本、发布程序和秘密恢复材料；数据库单独恢复、版本错配、摘要损坏和非空覆盖均被机制阻止；P360—P364共同覆盖故障、取消、重连、积压、可观测和恢复，M6退出条件满足 |
| 遗留问题 | 恢复材料在产品演练中为明确标识的非真实加密占位；客户PostgreSQL原生在线备份/PITR、KMS/Vault恢复、异地介质、HA、故障切换及RPO/RTO必须在M8按客户环境演练，当前结果不得表述为客户生产灾备结论 |
| 批准 | P364及M6完成，可进入M7产品基线测试与迁移切换 |
...
| 分支与提交 | `release/product-v2-test`；提交SHA以本记录所在提交为准 |
| 日期与执行人 | 2026-08-12；Codex执行 |
| 验证命令 | `JAVA_HOME=/tmp/askdata-jdk21 PATH=/tmp/askdata-jdk21/bin:$PATH bash scripts/v2-test-gate.sh` |
| 证据 | 入口主动清除客户数据库、迁移账号、Provider凭据、执行地址和服务令牌；隔离数据目录执行Python 57 tests、Java全量32 tests、冻结契约、一期三角色八场景33轮、二期三角色八场景33轮、Java→Python真实L1—L7 E2E、前端类型/生产/离线构建及外链扫描；12次DEMO Fixture执行均成功，均值1.133秒、p95 1.148秒、总计13.645秒；恢复演练包`v2-20260812T094815Z-d4a429b9`完成Flyway 15、五项摘要、发布包和秘密恢复材料一致性验证；最终输出`V2_TEST_GATE_PASS all customer_credentials=absent` |
| 结论 | Java/Python单元、数据库集成、契约、跨服务E2E、安全、性能冒烟、一期/二期回归、用户端离线构建和恢复回归已形成无客户凭据的一键门禁；测试分层与判定规则见`docs/PRODUCT_V2_TEST_STRATEGY.md` |
| 遗留问题 | 本任务性能数据仅为单机串行灾难性退化冒烟，不代表容量；200QPS、30/50执行并发、1300 SSE、30分钟峰值和8小时长稳在P372执行；真实客户Provider、数据库和网络仍属于M8 |
| 批准 | 满足P370退出条件，可进入P371迁移演练 |
...
| 任务ID | P372 |
| 状态 | IN PROGRESS |
| 分支与提交 | `release/product-v2-test`；短时容量检查点提交SHA以本记录所在提交为准 |
| 日期与执行人 | 2026-08-12；Codex执行 |
| 验证命令 | `JAVA_HOME=/tmp/askdata-jdk21 PATH=/tmp/askdata-jdk21/bin:$PATH bash scripts/v2-capacity-short.sh`；长测命令见`docs/PRODUCT_V2_CAPACITY_REPORT.md` |
| 已有证据 | 30执行槽接受30/30、P99 0.384秒；50执行槽接受50/50、P99 0.520秒；饱和后200请求全部在P99 0.612秒内明确429/503且返回`Retry-After: 1`；1300 SSE均收到3个有序事件、心跳和重连指引，无重复或丢失，RSS峰值532120 KiB、FD 1317、平台线程40；真实30分钟峰值运行1800.211秒，36,000/36,000成功、失败率0、提交P99 0.027秒、终态P95/P99 0.261/0.273秒、RSS峰值361496 KiB；最终代码Java 35 tests、Python 58 tests和冻结契约通过 |
| 结论 | 阶梯、30/50执行并发、200 QPS短突发、1300 SSE和30分钟峰值通过；当前仍不能标记P372通过 |
| 遗留问题 | 必须按真实时钟完成8小时长稳；客户认证、数据库、Provider、网关、多实例和生产容量仍在M8验证 |
| 批准 | 尚未批准；长测全部通过并补齐证据后再判定 |

## 12. 下一步

M0—M6及P370—P371已经完成，M7执行中。当前执行链为：

~~~text
P372 产品基线容量验证（短时通过，长测执行中）
  ↓
M7 发布、运维与产品化门禁
~~~
