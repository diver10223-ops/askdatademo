...
| 6 | M1—M7 产品基线 | IN PROGRESS | P372短时容量门禁已通过，真实30分钟峰值与8小时长稳待执行 |
| 7 | M8—M9 客户交付 | BLOCKED | 等待具体客户环境和验收输入 |

P300—P303已完成，M0门禁通过。稳定演示工作树位于`/workspaces/askdatademo-v1-v2-demo`，V2.0开发工作树位于`/workspaces/askdatademo`；M1—M6及P370—P371已完成，P372短时容量门禁通过，长时容量验证执行中。
...
| 已有证据 | 30执行槽接受30/30、P99 0.384秒；50执行槽接受50/50、P99 0.520秒；饱和后200请求全部在P99 0.612秒内明确429/503且返回`Retry-After: 1`；1300 SSE均收到3个有序事件、心跳和重连指引，无重复或丢失，RSS峰值532120 KiB、FD 1317、平台线程40；Java 32 tests、Python 57 tests和冻结契约通过 |
| 结论 | 阶梯、30/50执行并发、200 QPS短突发和1300 SSE短时门禁通过；当前仍不能标记P372通过 |
| 遗留问题 | 必须按真实时钟完成30分钟峰值和8小时长稳；客户认证、数据库、Provider、网关、多实例和生产容量仍在M8验证 |
