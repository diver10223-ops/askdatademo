...
本次没有沿用原“通过”结论，而是重新执行任务清单并补齐实测缺口：

- 新增可执行 POC 全矩阵，实际覆盖 3 角色、8 场景和 33 个必要轮次，并校验状态、终止层及短路后无多余层。
- 修复场景 7 补全请求仍返回 `WAITING_INPUT`，现在关联请求从 L1 重新执行至 L7。
- 修复全新数据目录启动时 Mock Warehouse 不写基线数据；真实单端口启动查询返回 1 行和 `980.50`。
- 修复离线构建扫描误报并禁用 module preload；单文件无外部 URL 资源或外部网络调用。
- Offline IndexedDB 升级至 v2，请求详情可在页面刷新后从持久存储恢复。
- 后台增加配置版本列表、Schema 边界校验导入、回滚，以及角色/资产/流程/场景/合规/运维资源 CRUD 与操作审计。
- 就绪检查现在使用项目虚拟环境、导入真实 FastAPI 应用并执行完整 POC 场景矩阵；测试依赖已锁定。

最终实测：`bash scripts/demo-readiness-check.sh` 通过；全新临时数据目录的 health、Session、Query、16 个 SSE 事件、`Last-Event-ID=10` 补发 11—16、查询详情、后台资源保存/读取及非法导入原子拒绝均通过。
