export class PocApiAdapter {
    ...
    createSession(role_id, options = {}) {
        return this.json("/sessions", {
            method: "POST",
            body: JSON.stringify({ role_id, ...options }),
        });
    }
    async query(session_id, question, scenario_id, parent_request_id, execution_variant = "POC") {
    ...
    async events(id, onEvent) {
        let last = Number(sessionStorage.getItem("askdata-event-" + id) || 0), attempts = 0, done = false;
        while (!done && attempts < 5) {
            await new Promise((resolve, reject) => {
                const es = new EventSource(`/api/v1/queries/${id}/events?last_event_id=${last}`);
                const names = [
                    "request.created",
                    "plan.created",
                    "task.created",
                ...
                names.forEach((n) => es.addEventListener(n, (x) => {
                    const m = x;
