...
const layers = ["L1", "L2", "L3", "L4", "L5", "L6", "L7"], names = [
    "交互层",
    "对话理解层",
    "语义层",
    "数据资产层",
...
export class OfflineAdapter {
    ...
    async session(id) {
        const value = await get("sessions", id);
        if (!value)
            throw new Error("离线Session不存在");
        return value;
    }
    async query(session_id, question, scenario, parent_request_id, _variant = "DEMO") {
        const id = crypto.randomUUID(), role = (await this.session(session_id)).role_id;
        if (scenario.startsWith("MT"))
            return this.v21(id, session_id, question, scenario, role, parent_request_id);
        const num = Number(scenario.split("-")[1]), sc = baseline.scenarios.find((x) => x.number === num), turn = sc.cases
            .find((x) => x.role_id === role)
            .turns.find((x) => question.includes("去年同期") || question.includes("为什么")
            ? x.turn === 2
            : x.turn === 1);
        const last = Number(turn.expected_last_layer.slice(1)), blocked = num === 5, waiting = num === 7 && !parent_request_id, status = blocked
            ? "BLOCKED"
            : waiting
                ? "WAITING_INPUT"
                : num === 2 || num === 3
        ...
        const detail = {
            ...
            sql_executions: status === "SUCCEEDED"
                ? [
                    {
                        sequence: 1,
    ...
    async v21(id, session_id, question, scenario, role, parent_request_id) {
        const org = role === "beijing" ? "BJ" : "HQ", allowed = role === "retail"
            ? ["retail_loan_issue_amt", "active_customer_count"]
            : ["loan_issue_amt", "deposit_balance", "npl_ratio", "fee_income"];
        let facts = v21.quarterlyFacts.filter((x) => x.org === org && allowed.includes(x.metric));
        if (["MT02", "MT05", "MT07"].includes(scenario))
            facts = facts.slice(0, 3);
        if (scenario === "MT08")
            facts = facts.slice(0, 2);
        const failedMetric = scenario === "MT06"
            ? "fee_income"
            : scenario === "MT07"
                ? "loan_issue_amt"
            ...
            code: `Q${i + 1}`,
            type: "SQL",
            critical: x.metric !== "fee_income" && x.metric !== "active_customer_count",
        ...
        const successful = queryTasks.filter((x) => x.status === "SUCCEEDED"), status = scenario === "MT07"
            ? "FAILED"
            : scenario === "MT06"
                ? "PARTIAL_SUCCESS"
                : "SUCCEEDED", tasks = [
    ...
    async detail(id) {
        ...
        if (memory)
            return memory;
        const saved = await get("requests", id);
        if (!saved)
            throw new Error("离线请求不存在");
